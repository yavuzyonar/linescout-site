...
- `index.html`, `betting.html`, `casino.html`, `news.html`, `about.html`, `disclosure.html`, `responsible-gambling.html`, `privacy.html`, `odds-calculator.html` — core pages
- `articles/` — one HTML file per article (evergreen guides and dated news briefs alike), plus `articles/manifest.json` which drives every listing page (`type`: `"evergreen"` or `"news"`)
...
This repo is written to automatically by a daily scheduled Claude task (see project notes) that publishes up to four items per day — one sports-betting evergreen guide, one casino evergreen guide, one sports-betting news brief, and one casino news brief (the two news briefs are skipped on days with no verifiable story) — and pushes them here directly, including the SEO pipeline steps above.
...
- `betting.html`, `casino.html`, and `news.html` are hand-written hub pages (intro copy, and for betting/casino a "start here" reading list) on top of the auto-generated, capped article grid — that intro content is not touched by any script.
